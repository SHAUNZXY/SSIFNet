...
def calc_lpips(img1, img2):
    ...
    img1 = torch.from_numpy(img1.astype(np.float32).transpose(2,0,1)[np.newaxis,])
    img2 = torch.from_numpy(img2.astype(np.float32).transpose(2,0,1)[np.newaxis,])

    lpips = loss_fn_alex(img1, img2).item()
...
def init_i3d_model():
    i3d_model_path = './load_model/i3d_rgb_imagenet.pt'
